"""
🌙 MonoQ Bot's Nice Functions - A collection of utility functions for trading
...
def cleanup_temp_data():
    if os.path.exists('temp_data'):
        print("🧹 MonoQ Bot cleaning up temporary data...")
...
def get_data(address, days_back_4_data, timeframe):
    ...
    if os.path.exists(temp_file):
        print(f"📂 MonoQ Bot found cached data for {address[:4]}")
    ...
    if response.status_code == 200:
        ...
        # Pad if needed
        if len(df) < 40:
            print(f"🌙 MonoQ Alert: Padding data to ensure minimum 40 rows for analysis! 🚀")
        ...
        print(f"📊 MonoQ's Data Analysis Ready! Processing {len(df)} candles... 🎯")

        # Always save to temp for current run
        df.to_csv(temp_file)
        print(f"🔄 MonoQ Bot cached data for {address[:4]}")
        ...
        print(f"❌ MonoQ Error: Failed to fetch data for address {address}. Status code: {response.status_code}")
...
def chunk_kill(token_mint_address, max_usd_order_size, slippage):
    ...
    cprint(f"\n🔪 MonoQ Bot's AI Agent initiating position exit...", "white", "on_cyan")
...
def ai_entry(symbol, amount):
    """AI agent entry function for MonoQ Bot's trading system 🤖"""
    cprint("🤖 MonoQ Bot's AI Trading Agent initiating position entry...", "white", "on_blue")
...
def get_token_balance_usd(token_mint_address):
    """Get the USD value of a token position for MonoQ Bot's wallet 🌙"""
    try:
