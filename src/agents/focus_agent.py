...
FOCUS_PROMPT = """
You are MonoQ Bot's Focus AI Agent. Analyze the following transcript and:
...
class FocusAgent:
    def __init__(self):
        ...
        if "deepseek" in self.active_model.lower():
            ...
            if deepseek_key:
                ...
                cprint("🚀 MonoQ Bot's Focus Agent using DeepSeek override!", "green")
            else:
                self.deepseek_client = None
                cprint("⚠️ DEEPSEEK_KEY not found - DeepSeek model will not be available", "yellow")
        else:
            self.deepseek_client = None
            cprint(f"🎯 MonoQ Bot's Focus Agent using Claude model: {self.active_model}!", "green")
        ...
        cprint("🎯 MonoQ Bot's Focus Agent initialized!", "green")
    ...
    def _check_schedule(self):
        ...
        if not (SCHEDULE_START <= current_time <= SCHEDULE_END):
            cprint(f"\n🌙 MonoQ Bot's Focus Agent is scheduled to run between {SCHEDULE_START.strftime('%I:%M %p')} and {SCHEDULE_END.strftime('%I:%M %p')}", "yellow")
    ...
    def _create_focus_log(self):
        ...
        cprint("🌟 MonoQ Bot's Focus History log created!", "green")
    ...
    def run(self):
        ...
        cprint("\n🎯 MonoQ Bot's Focus Agent starting with random monitoring...", "cyan")
